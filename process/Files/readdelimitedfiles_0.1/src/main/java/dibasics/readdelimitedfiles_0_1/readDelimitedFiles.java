...
package dibasics.readdelimitedfiles_0_1;
...
/**
 * Job: readDelimitedFiles Purpose: Import customers data<br>
 * Description: Data source is a CSV file <br>
 * 
 * @author user@talend.com
 * @version 7.3.1.20200219_1130
 * @status
 */
public class readDelimitedFiles implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "readDelimitedFiles.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(readDelimitedFiles.class);
	...
	private final String jobName = "readDelimitedFiles";
	...
	JobStructureCatcherUtils talendJobLog = new JobStructureCatcherUtils(jobName, "_Br0kIMJvEeyuzLpJyfGUKQ", "0.1");
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					readDelimitedFiles.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(readDelimitedFiles.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_DIBASICS_readDelimitedFiles = new byte[0];
		static byte[] commonByteArray_DIBASICS_readDelimitedFiles = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DIBASICS_readDelimitedFiles.length) {
					if (length < 1024 && commonByteArray_DIBASICS_readDelimitedFiles.length == 0) {
						commonByteArray_DIBASICS_readDelimitedFiles = new byte[1024];
					} else {
						commonByteArray_DIBASICS_readDelimitedFiles = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DIBASICS_readDelimitedFiles, 0, length);
				strReturn = new String(commonByteArray_DIBASICS_readDelimitedFiles, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DIBASICS_readDelimitedFiles) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "First", "Last", "Number", "Street", "City", "State", });
...
///////////////////////    			
					...
					class BytesLimit65535_tFileInputDelimited_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFileInputDelimited_1
									.append("FILENAME" + " = " + "\"C:/StudentFiles/DIBasics/Files/Customers.csv\"");
							...
							log4jParamters_tFileInputDelimited_1.append("TRIMSELECT" + " = " + "[{TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("First") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN="
									+ ("Last") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("Number") + "}, {TRIM="
									+ ("false") + ", SCHEMA_COLUMN=" + ("Street") + "}, {TRIM=" + ("false")
				...
				try {
					...
					Object filename_tFileInputDelimited_1 = "C:/StudentFiles/DIBasics/Files/Customers.csv";
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								"C:/StudentFiles/DIBasics/Files/Customers.csv", "ISO-8859-15", ",", "\n", true, 1, 0,
					...
					while (fid_tFileInputDelimited_1 != null && fid_tFileInputDelimited_1.nextRecord()) {
						...
						try {
							...
							row1.First = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);

							columnIndexWithD_tFileInputDelimited_1 = 1;

							row1.Last = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
...
///////////////////////		
							...
							if (row1.First != null) { //
								row_tLogRow_1[0] = String.valueOf(row1.First);

							} //

							if (row1.Last != null) { //
								row_tLogRow_1[1] = String.valueOf(row1.Last);

							} //

							if (row1.Number != null) { //
								row_tLogRow_1[2] = String.valueOf(row1.Number);

							} //

							if (row1.Street != null) { //
								row_tLogRow_1[3] = String.valueOf(row1.Street);

							} //

							if (row1.City != null) { //
								row_tLogRow_1[4] = String.valueOf(row1.City);

							} //

							if (row1.State != null) { //
								row_tLogRow_1[5] = String.valueOf(row1.State);
...
///////////////////////    			
					...
					if (!((Object) ("C:/StudentFiles/DIBasics/Files/Customers.csv") instanceof java.io.InputStream)) {
...
///////////////////////    			
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row1", 2, 0,
						talendJobLog, "tFileInputDelimited_1", "tFileInputDelimited", "tLogRow_1", "tLogRow",
						"output")) {
					talendJobLogProcess(globalMap);
				}
	...
	public static void main(String[] args) {
		final readDelimitedFiles readDelimitedFilesClass = new readDelimitedFiles();

		int exitCode = readDelimitedFilesClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'readDelimitedFiles' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'readDelimitedFiles' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = readDelimitedFiles.class.getClassLoader()
					.getResourceAsStream("dibasics/readdelimitedfiles_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = readDelimitedFiles.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : readDelimitedFiles");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 62176 characters generated by Talend Data Fabric on the April 22, 2022 at
 * 7:37:30 PM GMT
 ************************************************************************************************/